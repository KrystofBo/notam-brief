"""Latency of the same briefing requests on Nebius Token Factory and on Modal Shared Endpoints.

    python -m eval.latency --models deepseek-ai/DeepSeek-V4-Flash-0731 deepseek-ai/DeepSeek-V4.1-Flash \
        modal:deepseek-ai/DeepSeek-V4.1-Flash --repeat 2
...
is reported separately. The briefing requests are the real model-stage requests for the five labelled routes
(snapshot, pre-filter on, no weather).
...
def main():
    ...
    ap.add_argument("--models", nargs="+", required=True,
                    help="Token Factory ids; prefix 'modal:' for a Modal Shared Endpoint")
    ...
    for m in a.models:  # first contact: connection set-up, possible cold paths
    ...
    lines = ["# Latency: Nebius Token Factory vs Modal Shared Endpoints", "",
             ...
             f"reasoning `{config.REASONING_EFFORT}`). Modal: Shared Endpoint at {config.MODAL_BASE_URL}. Cost from "
             f"tokens at list prices (cached-input discounts not counted).", "",
    ...
    for m in a.models:
        ...
        lines.append(
            f"| {name.split('/')[-1]} | {'Modal Shared Endpoint' if provider == 'modal' else 'Token Factory'} | "
